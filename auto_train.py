...
bottlenecks, then auto-tunes and executes main.py with optimal parameters.
...
def main():
    parser = argparse.ArgumentParser("System & Bottleneck-Aware Smart Training Launcher")
    ...
    parser.add_argument('--model-type', type=str, default='enhanced', choices=['original', 'enhanced', 'transformer', 'dual', 'legacy'])
    parser.add_argument('--max-epoch', type=int, default=60, help="Total Phase-1 training epochs")
    parser.add_argument('--phase2-epochs', type=int, default=20, help="Phase-2 exploitation epochs")
    parser.add_argument('--save-dir', type=str, default='log/auto_run')
    ...
    print("\n" + "═"*75)
    print("  LegalSum System & Bottleneck-Aware Auto-Launcher")
    print("═"*75)
    ...
    print(f"\n[BOTTLENECK DIAGNOSIS & AUTO-TUNING]")
    auto_args = []
    ...
    if parsed_args.max_seq_len != -1:
        ...
        if not use_gpu and specs['total_ram_gb'] < 16.0:
            ...
            print(f"    -> Auto-tuning --max-seq-len=2000 to prevent RAM OOM explosion.")
            ...
            print(f"  • Sequence Length : Sufficient memory detected. Running full un-sampled sequence.")
    else:
    ...
    if not use_gpu and specs['total_ram_gb'] < 16.0:
        ...
        num_episode = 3
        ensemble_k = 5
        print(f"  • Model Scale     : CPU-tuned (hidden_dim=128, heads=4, episodes=3, K=5)")
    else:
        hidden_dim = 256
        num_heads = 8
        num_episode = 5
        ensemble_k = 10
        print(f"  • Model Scale     : Full Scale (hidden_dim=256, heads=8, episodes=5, K=10)")
    ...
    print("\n" + "═"*75)
    print("  Executing main.py with auto-tuned command:")
    print("  " + " ".join(cmd))
    print("═"*75 + "\n")
