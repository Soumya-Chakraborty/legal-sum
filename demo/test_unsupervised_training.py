...
def test_early_stopping_phase_transition(tmp_path):
    # 1. Setup mock features directory and files
    features_dir = tmp_path / "features"
    features_dir.mkdir()
    ...
    np.save(features_dir / "video1_visual.npy", np.random.randn(100, 1024))
    np.save(features_dir / "video1_acoustic.npy", np.random.randn(100, 40))
    np.save(features_dir / "video1_textual.npy", np.random.randn(100, 512))
    ...
    # Run main.py with --patience 2, max-epoch 15, phase2-epochs 5, --eval-courtroom
    cmd = [
        "python3", "main.py",
        "-d", "dummy",
        "-s", str(splits_path),
    ...
    # Assert successful exit code
    assert result.returncode == 0, f"Training failed with stderr: {result.stderr}"
    # Assert early stopping message was printed
    assert "Early stopping triggered" in result.stdout or "Early stopping triggered" in result.stderr
